...
def list_stories(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return db.query(DataStory).filter(DataStory.owner_id == current_user.id).all()
...
def get_story(story_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {
        "id": str(story.id), "title": story.title, "description": story.description,
        "theme": story.theme, "is_public": story.is_public, "share_token": story.share_token,
        "slides": [{"id": str(s.id), "title": s.title, "content": s.content,
                    "widget_id": str(s.widget_id) if s.widget_id else None,
                    "slide_order": s.slide_order, "layout": s.layout, "annotations": s.annotations} for s in slides]
    }
