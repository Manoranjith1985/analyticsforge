...
REPORT_FIELDS = ["id", "name", "description", "format", "is_scheduled", "schedule_cron"]

def _report_dict(r):
    return orm_to_dict(r, REPORT_FIELDS)
...
def list_reports(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return [_report_dict(r) for r in db.query(Report).filter(Report.owner_id == current_user.id).all()]
...
def create_report(data: ReportCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        db.rollback()
        import traceback
        raise HTTPException(status_code=500, detail=f"Create report error: {str(e)} | {traceback.format_exc()}")

@router.get("/{report_id}", response_model=ReportResponse)
def get_report(report_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return _report_dict(_get_or_404(report_id, current_user.id, db))

@router.delete("/{report_id}", status_code=204)
def delete_report(report_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    db.delete(_get_or_404(report_id, current_user.id, db))
    db.commit()

@router.get("/{report_id}/export")
def export_report(report_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def _get_or_404(report_id, user_id, db):
